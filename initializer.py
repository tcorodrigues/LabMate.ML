...
df.to_csv(os.path.join(init_files_dir,'all_combos.txt'), sep = ',', header = ['Pyridine', 'Aldehyde', 'Isocyanide', 'Temperature', 'Solvent', 'Catalyst', 'Time'], index=False)
...
df_random_data.to_csv(os.path.join(init_files_dir, 'train_data.txt'), sep= ',', header = ['Pyridine', 'Aldehyde', 'Isocyanide', 'Temperature', 'Solvent', 'Catalyst', 'Time'], index=False)
